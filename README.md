...
AngelHeart 是一个专为 [AstrBot](https://github.com/Soulter/AstrBot) 平台设计的智能群聊/私聊交互插件。它采用**两级 AI 协作**与**两态群聊参与模型**，在需要时回复，在不该打扰时保持安静。
...
- **离场 (NOT_PRESENT)**：只记录消息，不主动加入聊天。只有被 @ 或命中唤醒词才会进场；若开启离场应答，复读或密集聊天可以得到一次回复，但回复后仍保持离场。
- **在场 (OBSERVATION)**：被叫到时进入，在回复后继续保持。群聊暂时安静后，才由轻量模型判断是否需要接话；在场时间超过上限后，收到下一条消息时回到离场。

被叫到和普通群聊消息分别使用不同的等待规则；旧事件会被新的边界消息替换，避免针对半句话作答。
...
### ⏰ 回复节奏管理
- **主动唤醒入场**：离场时只有明确呼唤会让 AI 进入群聊；可选的复读或密集聊天应答只回复一次，不会变成持续参与。
- **两类等待**：被叫到时等待补充消息；在场时等群聊暂时安静后再判断是否接话。
- **避免连着插话**：回复或决定不回复后会暂缓下一轮普通判断；同一个群聊不会同时重复发起多轮是否接话的判断。
